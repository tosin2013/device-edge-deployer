...
GIT_URL := https://github.com/tosin2013/device-edge-demos.git
TAG := 0.1.0 
INSTALL_PATH = ~/.ansible-navigator.yml
SOURCE_FILE = ~/device-edge-demo/ansible-navigator/release-ansible-navigator.yml

INSTALL_ANSIBLE_NAVIGATOR := pip3 install ansible-navigator
BUILD_CMD := tag=$(TAG) && ansible-builder build -f execution-environment.yml -t device-edge-demo:$${tag} -v 3
...
REMOVE_IMAGES := podman rmi $$(podman images | grep "device-edge-demo" | awk '{print $$3}')
